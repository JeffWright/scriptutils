...
## Usage:
Some examples of what's here.  Not exhaustive.
...
```kotlin
    ...
    "ls $downloadsDir | grep a -"()
        .stdout(somefile) // Write output to file

    // Several ways to pass stdin to commands:
    "grep patch -"(somefile)
        .print() // Print stdout and stderr to screen
    // ...or...
    "grep patch -"()
        .stdin(somefile)
        .print()
    ...
    val month: String = listOf("Jan", "Feb", "Mar")
        .choose("Pick a month")
...
### Gradle:
```
implementation("com.github.JeffWright:scriptutils:<version>")
```
